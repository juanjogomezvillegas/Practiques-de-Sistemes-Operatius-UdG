...
int main(int argc, char *argv[]) {
	// Obre la pipe amb nom
	canal = open("./canal", O_WRONLY, 0);
	if (canal < 0) { // I si no ha pogut obrir la pipe, mostra un missatge d'error
		perror("Error al obrir la pipe amb nom");
		exit(1);
	}
	
	// I si tot va bé, es posa a llegir paraules del teclat mentre la cadena entrada no sigui XX (la cadena XX no s'enviarà a l'encripta)
	...
	while (!(strIsEqual(buff, FI))) { // Mentre sigui diferent de XX
		write(canal, buff, i); // Envia la paraula a l'encripta
	...
	close(canal); // Tanca el canal d'escriptura a la pipe amb nom
}
